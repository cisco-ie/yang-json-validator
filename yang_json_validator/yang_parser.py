...
"""
"""Core YANG parsing functionality.
This functionality should be largely deprecated via
https://github.com/mbj4668/pyang/pull/377
which has the majority of these convenience functions
present. However, the current methodology of parsing
statements seems to fail (no children) with pyang>=1.7.5
and thus we must continue using these utility functions
until resolved.
"""
...
def parse_repository(repository_path):
    ...
    file_repository = FileRepository(
        path=repository_path, use_env=False, no_path_recurse=False
    )
    context = Context(repository=file_repository)
    context.validate()
    logging.debug(
        "Found %d module(s) in %s.", len(context.revs.keys()), repository_path
    )
    return parse_modules(context)
